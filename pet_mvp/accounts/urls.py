...
from pet_mvp.accounts.views import RegisterOwnerView, logout_view, AccessCodeEmailView, \
    ...
    ApproveTempClinicView, BaseLoginView, ClinicDetailsView
...
urlpatterns = [
    ...
    path('owners/<int:pk>/', include([
        path('details/', OwnerDetailsView.as_view(), name='owner-details'),
        path('edit/', OwnerEditView.as_view(), name='owner-edit')
    ])),
    path('clinics/<int:pk>/', include([
        path('details/', login_not_required(ClinicDetailsView.as_view()), name='clinic-details'),
    ])),
